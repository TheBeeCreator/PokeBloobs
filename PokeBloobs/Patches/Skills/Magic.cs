...
namespace PokeBloobs.Patches.Skills
{
    internal class Magic
    {
        ...
        {
            if (_cachedMagicSouls != null) yield break;

            _cachedMagicSouls = new List<Item>();
            var Souls = PokeBloobs.SoulsDatabase.LoadedSouls
                            .Where(n => n.skillName.IndexOf("Magic", StringComparison.OrdinalIgnoreCase) >= 0);
        ...
        public class Patch_MagicSkill
        {
            static void Postfix(ref List<Item> ___petDrops)
            {
                ...
                {
                    Debug.Log("Magic souls not cached yet. Starting background cache...");
                ...
                else
                {
                    ...
                    {
                        Debug.Log($"Magic soul pet {soulItem.name} with rarity: {soulItem.dropChance} has been added to the queue and drop table");
