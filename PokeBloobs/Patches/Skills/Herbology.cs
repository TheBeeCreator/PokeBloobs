...
namespace PokeBloobs.Patches.Skills
{
    ...
    {
        public static List<Item> _cachedHerbologySouls;

        public static IEnumerator CacheSoulsCoroutine()
        {
            if (_cachedHerbologySouls != null) yield break;

            _cachedHerbologySouls = new List<Item>();
            var Souls = PokeBloobs.SoulsDatabase.LoadedSouls
                            .Where(n => n.skillName.IndexOf("Herblore", StringComparison.OrdinalIgnoreCase) >= 0);

            foreach (var soul in Souls)
            {
                Item c = PokeBloobs.BuildSoul(soul);
                c.dropChance = PokeBloobs.GetDropChance(soul.rarity);
                _cachedHerbologySouls.Add(c);
        ...
        public class Patch_HerbologySkill
        {
            static void Postfix(ref List<Item> ___petDrops)
            {
                ...
                if (_cachedHerbologySouls == null)
                {
                    Debug.Log("Herbology souls not cached yet. Starting background cache...");
                ...
                else
                {
                    ...
                    foreach (var soulItem in _cachedHerbologySouls)
                    {
                        Debug.Log($"Herbology soul pet {soulItem.name} with rarity: {soulItem.dropChance} has been added to the queue and drop table");
                ...
                Debug.Log($"Herbology soul drop patches have been applied");
            }
        }
    }
}
